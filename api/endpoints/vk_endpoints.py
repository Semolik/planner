...
@api_router.get('/settings', status_code=200, response_model=ChatsSettingsResponse, dependencies=[Depends(current_superuser)])
...
@api_router.put('/settings', status_code=200, dependencies=[Depends(current_superuser)], response_model=ChatsSettingsResponse)
async def update_settings(
    update_data: UpdateChatSettings,
    request: Request,
    db=Depends(get_async_session)
):
    settings_crud = SettingsCRUD(db)
    ...
    for role in ['photographers', 'copywriters', 'designers']:
        ...
        await settings_crud.set_setting(
            key=key,
            value='true' if getattr(update_data, f'vk_chat_{role}_enabled') else 'false')
