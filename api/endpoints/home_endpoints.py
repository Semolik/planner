...
async def get_home_notes(
    ...
    return await HomeCRUD(db).get_home_notes(
        ...
        else [UserRole[role.name] for role in UserRole]
    )


@api_router.post(
    "/notes", response_model=HomeNoteRead, dependencies=[Depends(current_superuser)]
)
async def create_home_note(
    text: str = Form(...), role: UserRole = Form(...), db=Depends(get_async_session)
):
    ...
    note = await HomeCRUD(db).create_home_note(text=text, role=role)
    return await HomeCRUD(db).get_note_by_id(note.id)


@api_router.put(
    "/notes/{note_id}",
    response_model=HomeNoteRead,
    dependencies=[Depends(current_superuser)],
)
async def update_home_note(
    note_id: uuid.UUID, text: str = Form(...), db=Depends(get_async_session)
):
    """
    ...
    return await home_crud.update_note(note=note, text=text)
...
async def add_file_to_home_note(
    note_id: uuid.UUID, file: UploadFile = File(...), db=Depends(get_async_session)
):
    home_crud = HomeCRUD(db)
    note = await home_crud.get_note_by_id(note_id)
    if not note:
    ...
    await home_crud.add_file_to_note(note_id=note.id, file_id=db_file.id)
    return db_file


@api_router.delete(
    "/notes/{note_id}", dependencies=[Depends(current_superuser)], status_code=204
)
async def delete_home_note(note_id: uuid.UUID, db=Depends(get_async_session)):
    """
...
@api_router.delete(
    "/notes/{note_id}/files/{file_id}",
    dependencies=[Depends(current_superuser)],
    status_code=204,
)
async def delete_file_from_home_note(
    note_id: uuid.UUID, file_id: uuid.UUID, db=Depends(get_async_session)
):
    """
    ...
    relation = await home_crud.get_note_file_relationship(
        note_id=note_id, file_id=file_id
    )
    if not relation:
        raise HTTPException(status_code=404, detail="Файл не найден в заметке.")
