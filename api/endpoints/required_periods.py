...
from schemas.periods import RequiredPeriod
from cruds.periods_crud import PeriodsCRUD
from schemas.periods import RoleConfig, CreateOrUpdatePeriodRequest

api_router = APIRouter(prefix="/required-periods", tags=["required periods"])

@api_router.get("",  dependencies=[Depends(current_user)], response_model=list[RequiredPeriod])
async def get_required_periods(
    db=Depends(get_async_session),
):
    return await PeriodsCRUD(db).get_periods()
@api_router.post("", response_model=RequiredPeriod, dependencies=[Depends(current_superuser)])
async def create_required_period(
    ...
    if create_data.period_start >= create_data.period_end:
        raise HTTPException(
            status_code=400, detail="Дата начала периода должна быть меньше даты конца периода"
        )
    periods_crud = PeriodsCRUD(db)
    if await periods_crud.get_period_from_range(create_data.period_start, create_data.period_end):
        raise HTTPException(
...
@api_router.get("/{period_id}", response_model=RequiredPeriod, dependencies=[Depends(current_user)])
...
@api_router.put("/{period_id}", response_model=RequiredPeriod, dependencies=[Depends(current_superuser)])
async def update_required_period(
    ...
    if not period:
        raise HTTPException(status_code=404, detail="Период не найден")
    existing_period = await periods_crud.get_period_from_range(update_data.period_start, update_data.period_end)
...
@api_router.delete("/{period_id}", status_code=204, dependencies=[Depends(current_superuser)])
