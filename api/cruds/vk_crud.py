...
from sqlalchemy import Subquery, select, true
...
class VKCRUD(BaseCRUD):
    async def create_chat(self, chat_id: int, chat_role: UserRole, name: str, members_count: int):

        chat = Chat(chat_id=chat_id, chat_role=chat_role,
    ...
    async def update_chat(self, chat: Chat, name: str, members_count: int, pin_message_id: int | None = None):
        chat.name = name
    ...
    async def get_chats_settings(self, vk_utils: 'VKUtils') -> ChatsSettingsResponse:
        ...
        copywriters_enabled_query = select(AppSettings.value).where(
            AppSettings.key == "vk_chat_copywriters_enabled"
        ).limit(1)
        designers_enabled_query = select(AppSettings.value).where(
            AppSettings.key == "vk_chat_designers_enabled"
        ).limit(1)
        token_query = select(AppSettings.value).where(
            AppSettings.key == "vk_token"
        ).limit(1)
        ...
        designer_chat_query = select(Chat).where(
            Chat.chat_role == UserRole.DESIGNER)

        # Execute queries
        photographers_enabled_result = await self.db.execute(photographers_enabled_query)
        ...
        photographers_enabled = bool(
            photographers_enabled_value == 'true') if photographers_enabled_value else False
        copywriters_enabled = bool(
            copywriters_enabled_value == 'true') if copywriters_enabled_value else False
        designers_enabled = bool(
            designers_enabled_value == 'true') if designers_enabled_value else False
        ...
        return ChatsSettingsResponse(
            ...
            photographers_chat=(await vk_utils.get_chat_by_id(photographer_chat.chat_id) if photographer_chat else None),
            copywriters_chat=(await vk_utils.get_chat_by_id(copywriter_chat.chat_id) if copywriter_chat else None),
            designers_chat=(await vk_utils.get_chat_by_id(designer_chat.chat_id) if designer_chat else None),
