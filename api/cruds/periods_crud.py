...
from models.user_models import RequiredPeriod, RolePeriodConfig
...
class PeriodsCRUD(BaseCRUD):
    ...
    async def get_period_by_id(self, period_id: uuid.UUID):
        query = select(RequiredPeriod).where(
            RequiredPeriod.id == period_id
        ).options(
            selectinload(RequiredPeriod.roles_config)
        )
        result = await self.db.execute(query)
        return result.scalars().first()
    async def create_period(self, period_start: date, period_end: date, photographers_count: int,
                            designers_count: int, copywriters_count: int):
