from vkbottle import bold, italic, url
...
def format_event(event: Event, include_description=True, include_photographers=True):
    ...
    if include_photographers:
        photographers_typed_task = next(
            (t for t in event.task.typed_tasks if t.task_type ==
             UserRole.PHOTOGRAPHER), None
        )
        if photographers_typed_task:
            users = [
                get_user_name(state.user) for state in photographers_typed_task.task_states
...
async def build_message(db: AsyncSession, role: UserRole) -> str:
    ...
    while True:
        ...
        new_filtered_events = [
            e for e in filtered_events if e.date <= two_weeks_later]
        ...
        new_filtered_events = [
            e for e in filtered_events if e.date <= one_week_later]
        if len(new_filtered_events) < len(filtered_events):
            filtered_events = new_filtered_events
            continue
