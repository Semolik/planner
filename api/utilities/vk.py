...
class VKUtils:
    ...
    def _build_event_info_message(self, event_data: dict) -> str:
        ...
        result_message += f"📍 Место: {event_data.get('location', 'не указано')}\n"
        ...
        async def handle_message_event(event: MessageEvent):
            ...
            if action == "process_announcement":
                try:
                    ...
                    # Если default level не найден, используем первый доступный
                    if not default_level and event_levels:
                        default_level = event_levels[0]
...
5. НАЗВАНИЕ СОБЫТИЯ должно быть официальным и лаконичным:
...
6. Для определения уровня мероприятия используй следующие подсказки:
   - ВАЖНО: Уровень ДОЛЖЕН быть ТОЛЬКО ИЗ ПРЕДОСТАВЛЕННОГО СПИСКА: {levels_list}
   - ИСКЛЮЧЕНИЕ: Только для СПЕКТАКЛЕЙ (театральных постановок) используй городской уровень
   - Для КОНЦЕРТОВ, выставок, ярмарок, фестивалей и других событий - используй уровень по умолчанию: "{default_level_name}"
   - Если в названии или описании явно упомянут один из доступных уровней - используй его
   - Если НИЧЕГО не подходит идеально - выбери БЛИЗКОЕ ПО СМЫСЛУ из доступного списка
   - Во всех остальных случаях используй уровень по умолчанию: "{default_level_name}"
   - ВАЖНО: Не путай концерты со спектаклями! Концерты, музыкальные события, фестивали → уровень по умолчанию
...
{{
  ...
  "level": "Один из доступных уровней"
}}
```
...
При невозможности точно идентифицировать поле оставляй его пустым (null). Для поля level используй уровень по умолчанию если не ясно какой выбрать."""
