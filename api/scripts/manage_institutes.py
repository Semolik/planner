...
@app.command("create")
def create_institute_cli(
    name: str = typer.Argument(..., help="Название нового института"),
):
...
async def _create_institute(name: str):
    async with get_async_session_context() as session:
        existing_institute = await session.execute(
...
@app.command("delete")
def delete_institute_cli(
    institute_id: str = typer.Argument(..., help="ID института для удаления"),
):
