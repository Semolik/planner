...
class AuditableMixin:
    ...
    def save_audit(self, session: Session, field_name, old_value, new_value, changed_by=None, change_type='UPDATE'):
        audit = AuditLog(
            ...
            field_name=self.field_labels.get(
                field_name, field_name),  # Используем русское название
    ...
    async def get_audit_history(self, session, tracked_fields=None, order_by="changed_at", descending=True):
        """
        ...
        query = (
            ...
            .order_by(getattr(AuditLog, order_by).desc() if descending else getattr(AuditLog, order_by).asc())
        )
        ...
        if tracked_fields:
            history.sort(key=lambda x: (
                ...
                tracked_fields.index(
                    x["field_name"]) if x["field_name"] in tracked_fields else len(tracked_fields)
            ), reverse=descending)
...
def register_audit_events(base_class, tracked_fields=None, order_by="changed_at", descending=True):
    ...
    def before_update_listener(mapper, connection, target):
        ...
        fields_to_track = tracked_fields or [
