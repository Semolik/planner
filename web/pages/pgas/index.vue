...
const reportForm = ref({
    kurs: 1,
    group: userData.value?.group || "", // ← добавить это
    is_magistracy: false,
    date: "",
});

// Предустановленные даты вступления
const joinDates = [
    "2024-11-16"
];
...
// Валидация формы справки
const isReportFormValid = computed(() => {
    return (
        reportForm.value.kurs >= 1 &&
        reportForm.value.kurs <= 5 &&
        reportForm.value.group.trim().length > 0 &&
        reportForm.value.date.length > 0
    );
});
function confirmReportMeetings() {
    if (!isReportFormValid.value) {
        $toast.error("Заполните все поля формы и выберите хотя бы одну планёрку");
        return;
    }

    // ✅ Формируем точную ссылку как в вашем примере
    const params = new URLSearchParams({
        year: selectedYear.value.toString(),
        kurs: reportForm.value.kurs.toString(),
        group: reportForm.value.group.trim(),
        is_magistracy: reportForm.value.is_magistracy.toString(),
        date: reportForm.value.date,
    });

    // Добавляем meetings_ids как отдельные параметры
    ...
    $toast.success("Справка скачивается...");
    closeReportModal();
}


function selectJoinDate(date: string) {
    reportForm.value.date = date;
}
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="flex gap-2 mt-1">
                        <app-button
                            ...
                            :active="isCreateFormValid && !isLoading"
                        >
                            Создать
                        </app-button>
            ...
            <template #body>
                ...
                <div v-else>
                    ...
                    <div class="grid grid-cols-2 gap-2 mb-4">
                        <app-input
                            v-model.number="reportForm.kurs"
                            label="Номер курса *"
                            type="number"
                            min="1"
                            max="5"
                    ...
                    <div class="mb-4">
                        <app-input
                            ...
                            label="Дата вступления в объединение *"
                            type="date"
                            white
                            required
                            class="w-full"
                    ...
                    <!-- Чекбокс магистратура -->
                    <div class="mb-6">
                        <UCheckbox
                            v-model="reportForm.is_magistracy"
                            label="Магистратура"
                            color="neutral"
                            size="lg"
                            variant="card"
